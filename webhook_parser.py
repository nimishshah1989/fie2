"""
FIE Phase 1 — TradingView Webhook Parser
Extracts maximum information from TradingView webhook payloads.

TradingView sends alerts as POST requests with a JSON or text body.
The alert message can contain TradingView placeholders like:
  {{ticker}}, {{exchange}}, {{close}}, {{open}}, {{high}}, {{low}},
  {{volume}}, {{time}}, {{timenow}}, {{interval}}, etc.

We also support custom JSON payloads embedded in alert messages for
indicator values and relative index data.
"""
...
# ─── TradingView Placeholder Mappings ──────────────────
...
# Known sector keywords for auto-classification
SECTOR_KEYWORDS = {
...
def parse_webhook_payload(raw_data: dict | str) -> dict:
    """
    Parse raw TradingView webhook payload into structured alert data.
    
    TradingView can send:
    1. Plain text message (with replaced placeholders)
    2. JSON body (if user configured JSON in alert message)
    3. Mixed: JSON embedded in text
    
    We support ALL formats and extract maximum data.
    """
    ...
    result = {
        ...
        "numerator_ticker": None,
        "denominator_ticker": None,
        "numerator_price": None,
...
def _extract_standard_fields(data: dict, result: dict):
    """Extract standard TradingView webhook fields"""
    
    # Direct field mapping (TradingView JSON format)
    field_map = {
        "ticker": "ticker",
        "symbol": "ticker",
        "exchange": "exchange",
    ...
    for src_key, dest_key in field_map.items():
        if src_key in data and data[src_key] is not None:
            ...
            if dest_key in ("price_open", "price_high", "price_low", "price_close", 
                          "price_at_alert", "volume"):
                try:
                    value = float(value)
                except (ValueError, TypeError):
                    pass
            if result[dest_key] is None:  # Don't overwrite if already set
                result[dest_key] = value
    
    # Set price_at_alert to close if not explicitly provided
...
def _extract_indicators(data: dict, result: dict):
    """Extract indicator values from payload"""
    ...
    # Check nested "indicators" object
    ...
    # Parse indicator values from message text
...
def _parse_indicators_from_text(text: str) -> dict:
    """Extract indicator values from free-text alert messages"""
    indicators = {}
    ...
    patterns = [
        ...
        (r"ADX[:\s]*(\d+\.?\d*)", "adx"),
        (r"ATR[:\s]*(\d+\.?\d*)", "atr"),
        (r"EMA\s*(\d+)[:\s]*(\d+\.?\d*)", None),  # Special handling
...
def _detect_relative_alert(data: dict, result: dict):
    """Detect if this is a relative (two-index) alert"""
    
    # Method 1: Explicit fields
    ...
    # Method 2: Ticker contains "/" or ":" indicating ratio
    ...
    # Method 3: Alert name or message contains ratio indicators
    ...
    for pattern in ratio_patterns:
        ...
        if match:
            ...
            # Validate they look like tickers (not common words)
    ...
    # For absolute alerts, set numerator = main ticker
...
def _classify_instrument(result: dict):
    """Auto-classify sector and asset class from ticker"""
    ...
    if result["asset_class"] is None:
        result["asset_class"] = "EQUITY"  # Default


def _interpret_signal(data: dict, result: dict):
    """Interpret signal direction from alert content"""
    
    # Check explicit signal field
    if "signal" in data:
    ...
    text = " ".join([
        ...
        str(data.get("alert_message", "")),
        str(data.get("alert_name", "")),
...
def _generate_summary(result: dict):
    ...
    if direction:
        emoji = "🟢" if direction == SignalDirection.BULLISH else "🔴" if direction == SignalDirection.BEARISH else "🟡"
        parts.append(f"{emoji} Signal: {direction.value}")
...
def get_recommended_alert_template() -> dict:
    """
    Returns the recommended JSON template for TradingView alert messages.
    Users should paste this into TradingView's alert message field.
    """
    return {
        "note": "Paste this into your TradingView Alert Message field. Replace indicator placeholders as needed.",
        "template": json.dumps({
            "ticker": "{{ticker}}",
            "exchange": "{{exchange}}",
            "interval": "{{interval}}",
            ...
            "time": "{{time}}",
            "timenow": "{{timenow}}",
            "alert_name": "YOUR_ALERT_NAME_HERE",
            "signal": "BULLISH_or_BEARISH",
            "indicators": {
                "rsi": "INDICATOR_VALUE",
                "macd": "INDICATOR_VALUE",
                "ema_200": "INDICATOR_VALUE"
            },
        ...
        "template_relative": json.dumps({
            ...
            "numerator": "NIFTYIT",
            "denominator": "NIFTY",
            "numerator_price": "NUMERATOR_PRICE",
            "denominator_price": "DENOMINATOR_PRICE",
            "ratio": "RATIO_VALUE",
            "signal": "BULLISH_or_BEARISH",
            "message": "YOUR_CUSTOM_MESSAGE"
        }, indent=2)
