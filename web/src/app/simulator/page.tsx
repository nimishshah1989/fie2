...
import type { BatchResponse, BatchRow, Strategy, SimulationResult } from "@/lib/simulator-types";
...
export default function SimulatorPage() {
  ...
  const handleFundClick1 = useCallback((code: string) => {
    ...
    if (code !== selectedFund1 && s1) {
      runSim(1, code, s1.metric, threshold1, multiplier1, sipAmount1, duration1);
    }
  }, [selectedFund1, s1, threshold1, multiplier1, sipAmount1, duration1, runSim]);

  const handleFundClick2 = useCallback((code: string) => {
    setSelectedFund2(code === selectedFund2 ? null : code);
    setSimResult2(null);
    if (code !== selectedFund2 && s2) {
      runSim(2, code, s2.metric, threshold2, multiplier2, sipAmount2, duration2);
    }
  }, [selectedFund2, s2, threshold2, multiplier2, sipAmount2, duration2, runSim]);
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-xs sm:text-sm text-slate-500 mt-1">
          ...
          1-month cool-off between top-ups &middot; Universe: Nifty 500
        </p>
      </div>
          ...
          {selectedFund1 && fund1 && (
            <div className="space-y-3">
              <SimulatorConfig
                ...
                running={running1}
                onRun={() => runSim(1, selectedFund1, s1.metric, threshold1, multiplier1, sipAmount1, duration1)}
          ...
          {selectedFund2 && fund2 && (
            <div className="space-y-3">
              <SimulatorConfig
                ...
                running={running2}
                onRun={() => runSim(2, selectedFund2, s2.metric, threshold2, multiplier2, sipAmount2, duration2)}
      ...
      {!loading && (
        <div className="bg-slate-50 rounded-lg p-3 text-xs text-slate-500 leading-relaxed">
          ...
          prevents consecutive top-ups within 30 days. Click any fund row to see the detailed
          chart and customize parameters.
        </div>
      )}
    </div>
  );
