...
export function ModelPortfolioDashboard() {
  const { portfolio, isLoading: loadingPortfolio } = useModelPortfolio();
  const { trades, isLoading: loadingTrades } = useModelTrades(20);
  const { navHistory, isLoading: loadingNav } = useModelNAV(365);
  const { performance, isLoading: loadingPerf } = useModelPerformance();
      ...
      <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
        ...
        {portfolio?.positions && portfolio.positions.length > 0 ? (
              ...
              <tbody>
                {portfolio.positions.map((p) => (
                  <tr key={p.instrument_id} className="border-b border-slate-50">
                    <td className="px-4 py-2.5 font-medium text-slate-900">{p.sector_name}</td>
                    <td className="px-3 py-2.5 text-slate-600">{p.instrument_id}</td>
          ...
          <div className="p-8 text-center text-sm text-slate-400">
            No open positions. Model portfolio will start trading when sectors enter LEADING + ACCUMULATION.
          </div>
        )}
      </div>
          ...
          <div className="divide-y divide-slate-50">
            {trades.slice(0, 10).map((t, i) => (
              <div key={i} className="flex items-center gap-3 px-4 py-2.5 text-sm">
                ...
                <span className="text-slate-500">({t.instrument_id})</span>
