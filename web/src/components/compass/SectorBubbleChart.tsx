...
function CustomTooltip({ active, payload }: { active?: boolean; payload?: Array<{ payload: SectorRS }> }) {
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-lg shadow-lg p-3 text-sm max-w-[280px]">
      ...
      <p className="text-xs text-slate-400 mt-1">Click to drill into stocks</p>
    </div>
  );
}
...
export function SectorBubbleChart({ sectors, onSectorClick }: Props) {
  const chartData = sectors.map((s) => ({
    ...
    x: s.rs_score,
    y: s.rs_momentum,
    z: s.pe_ratio ? Math.max(5, 100 - s.pe_ratio) : 30,
  }));
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 p-4">
        ...
        <ScatterChart margin={{ top: 20, right: 30, bottom: 20, left: 20 }}>
          ...
          >
            {chartData.map((entry, i) => (
              <Cell
                key={`cell-${i}`}
                fill={ACTION_COLORS[entry.action] || QUADRANT_COLORS[entry.quadrant]}
                fillOpacity={0.7}
                stroke={ACTION_COLORS[entry.action] || QUADRANT_COLORS[entry.quadrant]}
      ...
      {/* Legend */}
      <div className="flex items-center gap-4 px-6 -mt-2 flex-wrap">
        <div className="flex items-center gap-4 text-xs">
          <span className="flex items-center gap-1.5">
            <span className="w-3 h-3 rounded-full bg-emerald-600 inline-block" /> BUY
          </span>
          <span className="flex items-center gap-1.5">
            <span className="w-3 h-3 rounded-full bg-amber-600 inline-block" /> HOLD
          </span>
          <span className="flex items-center gap-1.5">
            <span className="w-3 h-3 rounded-full bg-blue-600 inline-block" /> WATCH
          </span>
          <span className="flex items-center gap-1.5">
            <span className="w-3 h-3 rounded-full bg-orange-600 inline-block" /> AVOID
          </span>
          <span className="flex items-center gap-1.5">
            <span className="w-3 h-3 rounded-full bg-red-600 inline-block" /> SELL
          </span>
        </div>
