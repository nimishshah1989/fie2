...
const ACTION_CONFIG: Record<CompassAction, {
  ...
  BUY: {
    ...
    label: "BUY", description: "All 3 gates pass — rising, outperforming, strengthening.",
  },
  HOLD: {
    bg: "bg-amber-50", text: "text-amber-700", border: "border-amber-200",
    label: "HOLD", description: "Momentum fading or downgraded by volume/regime.",
  },
  WATCH_EMERGING: {
    bg: "bg-blue-50", text: "text-blue-700", border: "border-blue-200",
    label: "WATCH — Emerging", description: "Rising but lagging market. Watch RS crossing 0.",
  },
  WATCH_RELATIVE: {
    bg: "bg-sky-50", text: "text-sky-700", border: "border-sky-200",
    label: "WATCH — Relative", description: "Outperforming but price falling. Watch for abs turn.",
  },
  WATCH_EARLY: {
    bg: "bg-indigo-50", text: "text-indigo-700", border: "border-indigo-200",
    label: "WATCH — Early", description: "Earliest reversal signal. Needs RS + price to confirm.",
  },
  AVOID: {
    bg: "bg-orange-50", text: "text-orange-700", border: "border-orange-200",
    label: "AVOID", description: "Rising but underperforming with fading momentum.",
  },
  SELL: {
    bg: "bg-red-50", text: "text-red-700", border: "border-red-200",
    label: "SELL", description: "Multiple gates failing. Underperforming and/or falling.",
  },
};
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
      {activeActions.map((action) => {
        ...
        const items = (grouped[action] || []).sort((a, b) => b.rs_score - a.rs_score);
        return (
          <div key={action} className="space-y-2">
            <div className="flex items-center gap-2 px-1">
              <span className={`${config.text} text-sm font-bold`}>{config.label}</span>
              <span className={`${config.text} opacity-60 text-xs`}>
                {items.length} sector{items.length !== 1 ? "s" : ""}
              </span>
            </div>
            <p className={`text-xs ${config.text} opacity-60 px-1`}>{config.description}</p>
...
function TableView({ sectors, onSectorClick }: { sectors: SectorRS[]; onSectorClick?: (k: string) => void }) {
  const sorted = [...sectors].sort((a, b) => b.rs_score - a.rs_score);
